...
import i5.las2peer.services.iStarMLModelService.dataStructures.CollectionData;
import i5.las2peer.services.iStarMLModelService.dataStructures.MailData;
import i5.las2peer.services.iStarMLModelService.dataStructures.ModelBuffer;
import i5.las2peer.services.iStarMLModelService.dataStructures.ResourceData;
import i5.las2peer.services.iStarMLModelService.dataStructures.ResourceVersion;
import i5.las2peer.services.iStarMLModelService.dataStructures.StringTuple;
import i5.las2peer.services.iStarMLModelService.dataStructures.XMLResponseElement;
import i5.las2peer.services.iStarMLModelService.dataStructures.XMLResponseElementChild;

import i5.las2peer.util.simpleRESTMapper.RESTMapper;
import i5.las2peer.util.simpleRESTMapper.Annotations.*;
...
public class IStarMLModelService extends Service
{
	...
	private void getConfig() throws Exception
	{
		try {
			...
			_sendMail=((String) _xPath.compile("//mail/@value").evaluate(_doc, XPathConstants.STRING)).toLowerCase().trim().equals("true");
	 ...
	 * @param shallow tree of data elements
	 * @return XML
	 * @throws Exception
	 */
	...
	/**
	 * Creates a XML as a String to return multiple data values to the service caller
	 * @param shallow trees of data elements
	 * @return XML
	 * @throws Exception
	 */
	private String createXMLResponse(XMLResponseElement[] data) throws Exception
	...
	private StringTuple getDBLogin() throws Exception
	{
		...
		String name=this.getContext().getLocalNode().getLoginForAgentId(userID);
	...
	public String listCollectionContents(@PathParam("collection") String collection,
			@DefaultValue("") @QueryParam("search") String searchQuery, @DefaultValue("model") @QueryParam("searchType") String searchType) throws Exception
	{
		cleanUp();
		...
		else//just output collection contents
		{
			...
			{				
				...
				respChild.addAttribute("name",cols[i].getName());
				respChild.addAttribute("owner",cols[i].getOwner());				
			...
			{				
				XMLResponseElementChild respChild=new XMLResponseElementChild("Resource");
				respChild.addAttribute("name",res[i].getName());
				respChild.addAttribute("lastModified", res[i].getLastModified().toString());	
			...
			@PathParam("resource") String resource) throws Exception
	{
		...
		{				
			...
			respChild.addAttribute("revision",  Integer.toString(versionData[i].getRevision()));
			respChild.addAttribute("date", versionData[i].getDate().toString());
			respChild.addAttribute("user", versionData[i].getUser());
			respRes.addChild(respChild);				
		}
	...
	IStarMLEditor editorr=new IStarMLEditor();
	/**
	...
	{		
		...
		if(nodeType.equals(ACTOR_NODE_TYPE))
		{						
			editor.createActor(nodeID,name, type, comment);			
		}
		else if(nodeType.equals(IELEMENT_NODE_TYPE))
		{
	...
	public String createActor(@PathParam("collection") String collection,
			...
			@DefaultValue("Actor") @QueryParam("name") String name, @DefaultValue("actor") @QueryParam("type") String type, @DefaultValue("") @QueryParam("comment") String comment) throws Exception
	{
	...
	public String createIElement(@PathParam("collection") String collection,
			@PathParam("resource") String resource, @PathParam("nodeID") String nodeID,
			@DefaultValue("Goal") @QueryParam("name") String name, @DefaultValue("goal") @QueryParam("type") String type, @DefaultValue("") @QueryParam("comment") String comment) throws Exception
	{
	...
	public String createActorLink(@PathParam("collection") String collection,
			...
			@DefaultValue("is_a") @QueryParam("type") String type) throws Exception
	{
